...
export default function ShoulderPage() {
  ...
  const bodyPart: BodyPart = "shoulder";
  const info = BODY_PART_INFO[bodyPart];

  const [calibration, setCalibration] = useState<ShoulderCalibrationProfile | null>(null);
  ...
  useEffect(() => {
    ...
    if (shoulderProfile) {
      setCalibration(JSON.parse(shoulderProfile));
    } else {
      router.push("/shoulder/calibrate");
    }
  }, [router]);

  const [confidence, setConfidence] = useState(7);
  const [movementRestrictions, setMovementRestrictions] = useState<ShoulderMovementRestriction[]>([]);
  const [sensations, setSensations] = useState<ShoulderSensation[]>([]);
  const [restingDiscomfort, setRestingDiscomfort] = useState(0);
  const [activityGoal, setActivityGoal] = useState<ActivityGoal>("training");
  const [painLocation, setPainLocation] = useState<ShoulderPainLocation[]>([]);
  ...
  const [expandedMovementCategories, setExpandedMovementCategories] = useState<string[]>(["Reaching"]);

  const needsPainLocation = sensations.some(s => {
    const info = SHOULDER_SENSATION_INFO[s];
    return info?.category === "pain" || info?.warning || info?.danger;
  });

  const readiness: ShoulderReadiness = useMemo(() => ({
    confidence,
    ...
    sleptOnShoulder,
    problemZoneStatus,
  }), [confidence, movementRestrictions, sensations, restingDiscomfort, activityGoal, painLocation, needsPainLocation, sleptOnShoulder, problemZoneStatus]);

  const coach = useMemo(() => initCoachState(bodyPart, readiness), [readiness]);

  function toggleSensation(s: ShoulderSensation) {
    if (s === "nothing" || s === "good") {
  ...
  function toggleMovement(m: ShoulderMovementRestriction) {
    setMovementRestrictions((prev) => prev.includes(m) ? prev.filter((x) => x !== m) : [...prev, m]);
  }

  function togglePainLocation(loc: ShoulderPainLocation) {
  ...
  function startSession() {
    localStorage.setItem("bodyCoach.shoulder.readiness", JSON.stringify(readiness));
    localStorage.setItem("bodyCoach.shoulder.coachState", JSON.stringify(coach));
    router.push("/shoulder/session");
  }
  ...
  return (
    <main style={{ maxWidth: 560, margin: "0 auto", padding: 16, fontFamily: "system-ui" }}>
      ...
      <div className="app-header">
        ...
        <Link href="/shoulder/calibrate" className="btn" style={{ fontSize: 13 }}>⚙️ Profile</Link>
      </div>
      ...
      {calibration.problemZones && calibration.problemZones.length > 0 && (
        <div className="card" style={{ marginTop: 12 }}>
          ...
          <p className="muted" style={{ margin: "4px 0 12px 0", fontSize: 13 }}>
            {calibration.problemZones.map(pz => SHOULDER_ROM_ZONES[pz.zoneIndex]?.label).join(", ")}
          </p>
      ...
      {/* Confidence */}
      <div className="card" style={{ marginTop: 12 }}>
        <div className="section-header">How much do you trust your shoulder right now?</div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        {SHOULDER_SENSATION_CATEGORIES.map((category) => {
        ...
        <div className={`adaptive-section ${needsPainLocation ? "visible" : ""}`}>
          ...
          {SHOULDER_PAIN_LOCATION_CATEGORIES.map((category) => (
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        {SHOULDER_MOVEMENT_CATEGORIES.map((category) => {
      ...
      {/* Resting Discomfort */}
      <div className="card" style={{ marginTop: 12 }}>
        <div className="section-header">Any discomfort right now, arm at rest?</div>
        <div className="slider-container">
          <div style={{ display: "flex", alignItems: "baseline" }}>
